...
class hr_employee(models.Model):
    ...
    type_ids = fields.Char(string="Degree")
        ...
        if not self.job_id:
            ...
            if not self.emp_entry:
                ...
                self.write({'entry_id': emp_entry_id.id,
                            'emp_entry': True})
                employee_action = self.env.ref('checklist.action_aspl_emp_entry_for_recruitment')
    ...
    def extend_period(self):
        view_id = self.env.ref('checklist.employee_training_history_form_view').id
        return {
