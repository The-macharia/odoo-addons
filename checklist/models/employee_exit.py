...
class employee_exit(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'),
                              ('sent_admin', 'Sent to Departmental Clearance'),
                              ('sent_production', 'Sent to Prodcution/Workshop Manager'),
                              ('sent_hr', 'Sent to HR'),
                              ('verified', 'Verified'),
                              ('exit_done', 'Closed'),
    ...
    departmental_clearance = fields.Char(string="Departmental Clearance")
    assignment_completed = fields.Text(string="Assignment Completed")
    assignment_pending = fields.Text(string="Assignment Pending")
    equipment_returned = fields.Text(string="Tools/ Equipment Returned ")
    line_manager = fields.Many2one('hr.employee', string="Line Manger")
    line_manager_approve = fields.Char(string="Line Manger Signature")
    date_linemanager_approve = fields.Date(string="Date Line Manager Approved")
    comments = fields.Text(string="Production /Workshop Manager Comments")
    workshop_manager = fields.Many2one('hr.employee', string="Workshop Manager")
    workshop_manager_approve = fields.Char(string="Workshop Manager Signature")
    date_workshopmanager_approve = fields.Date(string="Date Workshop Manager Approved")
    store_manager = fields.Many2one('hr.employee', string="Store Manager")
    store_manager_approve = fields.Char(string="Store Manager Signature")
    date_storemanager_approve = fields.Date(string="Date Store Manager Approved")
    hr_admin = fields.Many2one('hr.employee', string="HR/ ADMIN")
    hr_admin_approve = fields.Char(string="HR/ ADMIN Signature")
    date_hr_admin_approve = fields.Date(string="Date HR/ ADMIN Approved")
    outstanding_loans = fields.Char(string="Outstanding Loans")
    outstanding_fines = fields.Char(string="Outstanding Fines")
    salary_advance = fields.Char(string="Salary Advance")
    other_comments = fields.Text(string="Other Comments")
    employee_signature = fields.Char(string="Employee’s Signature ")
    date_employee_sign = fields.Date(string="Date")
    manager_appr_req = fields.Boolean('Require Line Approval?')
    ...
    basis = fields.Selection([('retirement', 'RETIREMENT'),
    ...
    def check_notice_period(self):
        ...
        if self.manager_appr_req and self.state in ('sent_admin', 'sent_production','sent_hr') \
    ...
    @api.one
    def sent_production(self):
        self.state='sent_production'    
    ...
    def verify_resignation(self):
        ...
        template = self.env.ref('checklist.email_emp_notice_period_template')
        if template:
    ...
    def action_makeMeeting_custom(self):
        ...
        if self.env.context.get('from_exit'):
            ...
            res = self.env['ir.actions.act_window'].for_xml_id('checklist', 'action_calendar_event_from_employee_form')
            res['context'] = {
