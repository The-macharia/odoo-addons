...
#         loan_obj = self.env['account.loan']
        if not (self._uid == SUPERUSER_ID):
            self._cr.execute('select count(id) from account_loan where company_id={}'.format(
...
#             loan_count = loan_obj.sudo().search([('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='draft'".format(
                self.env.user.company_id.id))
...
#             new_loan = loan_obj.sudo().search([('state', '=', 'draft'),('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='done'".format(
                self.env.user.company_id.id))
...
#             loan_done_count = loan_obj.sudo().search([('state', '=', 'done'),('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='apply'".format(
                self.env.user.company_id.id))
...
#             loan_sanctioned_count = loan_obj.sudo().search([('state', '=', 'apply'),('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='approved'".format(
                self.env.user.company_id.id))
...
#             loan_disbursed_count = loan_obj.sudo().search([('state', '=', 'approved'),('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='partial'".format(
                self.env.user.company_id.id))
...
#             loan_partial_count = loan_obj.sudo().search([('state', '=', 'partial'),('company_id','=',self.env.user.company_id.id)])
            self._cr.execute("select count(id) from account_loan where company_id={} and state='cancel'".format(
                self.env.user.company_id.id))
...
#             loan_count = loan_obj.sudo().search([])\
            self._cr.execute(
                "select count(id) from account_loan where state='draft'")
            new_loan1 = self._cr.fetchone()
            if new_loan1:
                new_loan = new_loan1[0]
            else:
...
#             new_loan = loan_obj.sudo().search([('state', '=', 'draft')])
            self._cr.execute(
                "select count(id) from account_loan where state='done'")
            loan_done_count1 = self._cr.fetchone()
            if loan_done_count1:
                loan_done_count = loan_done_count1[0]
            else:
...
#             loan_done_count = loan_obj.sudo().search([('state', '=', 'done')])
            self._cr.execute(
                "select count(id) from account_loan where state='apply'")
            loan_sanctioned_count1 = self._cr.fetchone()
            if loan_sanctioned_count1:
                loan_sanctioned_count = loan_sanctioned_count1[0]
            else:
...
#             loan_sanctioned_count = loan_obj.sudo().search([('state', '=', 'apply')])
            self._cr.execute(
                "select count(id) from account_loan where state='approved'")
            loan_disbursed_count1 = self._cr.fetchone()
            if loan_disbursed_count1:
                loan_disbursed_count = loan_disbursed_count1[0]
            else:
...
#             loan_disbursed_count = loan_obj.sudo().search([('state', '=', 'approved')])
            self._cr.execute(
                "select count(id) from account_loan where state='partial'")
            loan_partial_count1 = self._cr.fetchone()
            if loan_partial_count1:
                loan_partial_count = loan_partial_count1[0]
            else:
...
#             loan_partial_count = loan_obj.sudo().search([('state', '=', 'partial')])
            self._cr.execute(
                "select count(id) from account_loan where state='cancel'")
            loan_cancel_count1 = self._cr.fetchone()
            if loan_cancel_count1:
                loan_cancel_count = loan_cancel_count1[0]
            else:
...
#             loan_cancel_count = loan_obj.sudo().search([('state', '=', 'cancel')])
    ...
    def _get_total_amount(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search([('state', 'in', [
                'partial', 'approved', 'done']), ('company_id', '=', self.env.user.company_id.id)])
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['partial', 'approved', 'done'])])
        total = 0.00
        cur_id = None
...
#                 total += sum(l1)
        if cur_id:
            ...
            #             else:
            self.total_amt = formatLang(
                self.env, cur_id.round(total) + 0.0, currency_obj=cur_id)
        else:
            self.total_amt = 0.00
    ...
    def _get_loan_decline_amt(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['cancel']), ('company_id', '=', self.env.user.company_id.id)])
            cur_id = self.env.user.company_id.currency_id
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['cancel'])])
            cur_id = self.env['res.currency'].search(
                [('name', '=', 'RWF'), ('symbol', '=', 'RWF')], limit=1)
        total = 0.00
        for loan in loan_obj:
            #             cur_id = None
            #             if loan.journal_disburse_id:
...
#         cur_id = self.env['res.currency'].search([('name', '=', 'RWF'),('symbol', '=', 'RWF')], limit=1)
        if cur_id:
            self.loan_cancel_total = formatLang(
                self.env, cur_id.round(total) + 0.0, currency_obj=cur_id)
        else:
    ...
    def _get_loan_partially_amt(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['partial']), ('company_id', '=', self.env.user.company_id.id)])
            cur_id = self.env.user.company_id.currency_id
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['partial'])])
...
#         cur_id = self.env['res.currency'].search([('name', '=', 'RWF'),('symbol', '=', 'RWF')], limit=1)
        if cur_id:
            self.loan_partial_total = formatLang(
                self.env, cur_id.round(total) + 0.0, currency_obj=cur_id)
        else:
    ...
    def _get_loan_done_amt(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['done']), ('company_id', '=', self.env.user.company_id.id)])
            cur_id = self.env.user.company_id.currency_id
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['done'])])
...
#         cur_id = self.env['res.currency'].search([('name', '=', 'RWF'),('symbol', '=', 'RWF')], limit=1)
        if cur_id:
            self.loan_done_total = formatLang(
                self.env, cur_id.round(total) + 0.0, currency_obj=cur_id)
        else:
    ...
    def _get_loan_disbursed_amt(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['approved']), ('company_id', '=', self.env.user.company_id.id)])
            cur_id = self.env.user.company_id.currency_id
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'in', ['approved'])])
...
#         cur_id = self.env['res.currency'].search([('name', '=', 'RWF'),('symbol', '=', 'RWF')], limit=1)
        if cur_id:
            self.loan_disbursed_total = formatLang(
                self.env, cur_id.round(total) + 0.0, currency_obj=cur_id)
        else:
...
#
        ...
        for loan in loan_obj:
            for dis in loan.disbursement_details:
                if str(dt.year) in dis.bill_date.strftime('%d/%m/%Y'):
                    if dis.release_number.state == 'posted':
                        for line in dis.release_number.line_ids:
                            present += line.credit
                elif str(dt.year - 1) in dis.bill_date.strftime('%d/%m/%Y'):
                    if dis.release_number.state == 'posted':
                        for line in dis.release_number.line_ids:
                            ps1 += line.credit
                elif str(dt.year - 2) in dis.bill_date.strftime('%d/%m/%Y'):
                    if dis.release_number.state == 'posted':
                        for line in dis.release_number.line_ids:
                            ps2 += line.credit
                elif str(dt.year - 3) in dis.bill_date.strftime('%d/%m/%Y'):
                    if dis.release_number.state == 'posted':
                        for line in dis.release_number.line_ids:
                            ps3 += line.credit
                else:
    ...
    def get_realization_datas(self):
        ...
        if realization_graph_flag == False:
            ...
            start_date = datetime.strptime(
                str(date.today()), "%Y-%m-%d").date()
            end_date = date.today() + relativedelta(months=1)
#             relativedelta(months=+int(self.pro_month))
#             end_date = datetime.strptime(str(end_date), "%Y-%m-%d").date()
            if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
                loan_obj = self.env['account.loan'].sudo().search(
                    [('state', 'in', ['partial', 'approved']), ('company_id', '=', self.env.user.company_id.id)])
            else:
                loan_obj = self.env['account.loan'].sudo().search(
                    [('state', 'in', ['partial', 'approved'])])
            cur_id = None
            prin_am_tot = 0.0
            for loan in loan_obj:
...
#                 installment_ids = self._cr.fetchall()
                ...
                self._cr.execute("select id from payment_schedule_line where date >= '{}' and date <= '{}' and loan_id = '{}'".format(
                    start_date, end_date, loan.id))
                ...
                if schedule_ids:
                    for o in schedule_ids:
                        ...
                        for i in installment_ids:
                            if i:
                                self._cr.execute(
                                    "select sum(outstanding_prin), sum(outstanding_int), sum(outstanding_fees) from account_loan_installment where id = %s" % i[0])
                ...
                if cur_id and cur_id.id != loan.company_id.currency_id.id:
                    local_exp_pri = cur_id.with_context(date=current_date).compute(
                        local_total_principle, loan.company_id.currency_id)
                    local_exp_int = cur_id.with_context(date=current_date).compute(
                        local_total_interest, loan.company_id.currency_id)
                    local_exp_fees = cur_id.with_context(date=current_date).compute(
                        local_total_fees, loan.company_id.currency_id)
...
#             print (aa,'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
            ...
            # Set Fields
            self.total_principle = formatLang(
                self.env, round(total_principle, 2))
            ...
            realization_graph_data = [
                {'values': data, 'title': '', 'key': _('Realization')}]
            return realization_graph_data
        else:
            if rel_currency:
                self.total_principle = formatLang(self.env, rel_currency.round(
                    final_total_principle) + 0.0, currency_obj=rel_currency)
                self.total_interest = formatLang(self.env, rel_currency.round(
                    final_total_interest) + 0.0, currency_obj=rel_currency)
                self.total_fees = formatLang(self.env, rel_currency.round(
                    final_total_fees) + 0.0, currency_obj=rel_currency)
            else:
        ...
        if out_graph_flag == False:
            ...
            if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
                loan_obj = self.env['account.loan'].sudo().search([('state', 'in', ['partial', 'approved', 'done']), (
                    'approve_date', '<=', datetime.strptime(today, "%Y-%m-%d")), ('company_id', '=', self.env.user.company_id.id)])
            else:
                loan_obj = self.env['account.loan'].sudo().search([('state', 'in', [
                    'partial', 'approved', 'done']), ('approve_date', '<=', datetime.strptime(today, "%Y-%m-%d"))])
            cur_id = None
            for loan in loan_obj:
                cur_id = None
                tot_bal_pri = 0.0
...
#                     date_inst = datetime.strptime(inst.date, "%Y-%m-%d")
                    ...
                    schedule_line_ids = self.env['payment.schedule.line'].search(
                        [('date', '<=', datetime.strptime(today, "%Y-%m-%d")), ('installment_id', 'in', [inst.id])])
                    if schedule_line_ids:
                        l1 = [
                            l.capital for l in schedule_line_ids.installment_id if inst.id == l.id]
                        tot_prin += sum(l1)
                        if arrear_day > 30:
...
#                                             bal_due_prin += l.due_principal
                ...
                if cur_id and cur_id.id != loan.company_id.currency_id.id:
                    pri += cur_id.with_context(date=date_new).compute(
                        bal_pri, loan.company_id.currency_id)
                    intr += cur_id.with_context(date=date_new).compute(
                        bal_int, loan.company_id.currency_id)
                    fees += cur_id.with_context(date=date_new).compute(
                        bal_fees, loan.company_id.currency_id)
                    late_fees += cur_id.with_context(date=date_new).compute(
                        late_fee, loan.company_id.currency_id)
                    due_prin += cur_id.with_context(date=date_new).compute(
                        bal_due_prin, loan.company_id.currency_id)
                    if bal_lst_30_prin:
                        par30 += cur_id.with_context(date=date_new).compute(
                            bal_lst_30_prin, loan.company_id.currency_id)
                else:
                    pri += bal_pri
                    intr += bal_int
                    fees += bal_fees
            ...
            if cur_id:
                currency = cur_id
                self.total_out_amt = formatLang(self.env, cur_id.round(
                    total_outstanding_amt) + 0.0, currency_obj=cur_id)
            else:
            ...
            out_graph_data = [{'values': data, 'due_prin': due_prin,
                               'par_30': par30, 'title': '', 'key': _('Outstanding Amount')}]
            return out_graph_data
        else:
            if currency:
                self.total_out_amt = formatLang(self.env, currency.round(
                    total_outstanding_amt) + 0.0, currency_obj=currency)
            else:
                self.total_out_amt = total_outstanding_amt
            return out_graph_data
...
#                 dis_tot += sum(l1)
            if loan.partner_id.business_industry_id:
                if loan.partner_id.business_industry_id.name in sec_dict:
                    ...
                    sec_dict.update(
                        {loan.partner_id.business_industry_id.name: dis_tot})
            else:
    ...
    def get_gender_dict(self):
        ...
        if not (self._uid == SUPERUSER_ID or self.env.user.has_group('base.group_system')):
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'not in', ['cancel']), ('company_id', '=', self.env.user.company_id.id)])
        else:
            loan_obj = self.env['account.loan'].sudo().search(
                [('state', 'not in', ['cancel'])])
        ...
        for loan in loan_obj:
            if loan.approve_date:
                # appr_date = loan.approve_date.strftime('%Y-%m-%d')
                appr_date = loan.approve_date
...
#                     due_due = data_line['value']
    ...
    def _get_gender_percent(self):
        ...
        for rec in self:
            try:
                rec.gender_percent = round(
                    (float(gen['cur'])/float(gen['all_loan']))*100, 2)
            except ZeroDivisionError:
