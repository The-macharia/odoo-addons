...
class LoanPayment(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not 'transfer_payment_active_id' in self._context:
            active_id = self._context.get('active_id')
        else:
            active_id = self._context.get('transfer_payment_active_id')
...
#         print (self._context,'========================12323')
        if 'new_loan_partner_id' in self._context and self._context.get('new_loan_partner_id'):
            ...
            tr_journal_id = self.journal_id.id
            trn_new_loan_ptr = self.loan_id.partner_id.id
...
#                             break
        ...
        if 'from_payment_confirm_button' not in self._context:
            try:
                ...
                if currency_id.id != company_curren.id:
                    val = self.get_currency_diff_vals(move_vals)
                    if val:
                        move_id = self.env['account.move'].create(move_vals)
        ...
        for line in mv_vals['line_ids']:
            if diff_credit:
                if line[2]['credit'] and line[2]['credit'] > 1:
                    if diff_credit > 0.0 and diff_credit < 1:
                        line[2]['credit'] = line[2]['credit'] - diff_credit
                    break
            elif diff:
                if diff > 0.0 and diff < 1:
                    if line[2]['debit'] and line[2]['debit'] > 1:
                        line[2]['debit'] = line[2]['debit'] - diff
        return mv_vals
                
#         elif debit_total > credit_tot:
#             diff =  debit_total - credit_tot
#             if diff > 0.0 and diff < 1:
#                 debit_total += 0.01
