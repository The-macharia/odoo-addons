...
class PaymentReceiptWizard(models.TransientModel):
    ...
    def get_max_Date(self):
        ...
        final_date = ''
        if max_date:
            final_date = datetime.strptime(max_date, "%Y-%m-%d").strftime('%m/%d/%Y')
        return final_date
    ...
    def default_get(self, fields_list):
        ...
        res.update({'release_ids_domain':json.dumps([('id','in',[i.release_number.id for i in loan_obj.repayment_details])])})
        return res
    ...
    def print_payment_receipt(self):
        ...
        datas = {}
        print ("\n\n\nvalues----------------------",self._context)
        if self._context.get('active_id'):
            loan_obj = self.env['account.loan'].browse(self._context.get('active_id'))
            datas.update({'loan_obj':loan_obj})
        print('\n\n\n\nDatassssssssssssssssssssssssss',type(datas),datas)
        return self.env.ref('pragtech_loan.payment_receipt_id').report_action(self)
